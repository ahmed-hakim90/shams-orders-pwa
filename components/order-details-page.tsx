        ...
        <div className="order-layout">
          ...
          <aside className="order-side-column">
            <section className="detail-card operations-card">
              ...
              <fieldset className="status-picker" disabled={saving || !order.branch}><legend>اختار حالة التشغيل</legend>{operationalStatuses.map((status) => { const unavailable = order.status !== status.value && !allowedStatuses.includes(status.value); return <button type="button" key={status.value} className={order.status === status.value ? "is-selected" : ""} aria-pressed={order.status === status.value} disabled={saving || unavailable || order.status === status.value} title={unavailable ? "الانتقال للحالة دي غير متاح من الحالة الحالية" : undefined} onClick={() => changeStatus(status.value)}><span><i/>{status.label}</span><small>{order.status === status.value ? "الحالة الحالية" : unavailable ? "غير متاح حاليًا" : status.description}</small></button>; })}</fieldset>
              <p className="field-notice">حالة التشغيل مستقلة عن الدفع؛ بدء التجهيز لا يعني إن الأوردر اتدفع.</p>
